...
#pragma warning restore 618
	...
	public class OptionSet : KeyedCollection<string, Option>
	{
		...
		public List<string> Parse (IEnumerable<string> args, bool includeEnvironment)
		{
			if (args == null)
			{
			...
			if (includeEnvironment)
			{
				foreach (var item in Items)
				{
					foreach (var name in item.Names)
					{
						...
						{
							var argument = "--" + name + "=" + variable;
							arguments.Add(argument);
							Bro.Log.Info("options :: argument passed = " + argument);
						}
					}
				}
			}
		...
		private const int Description_FirstWidth  = 180 - OptionWidth;
		private const int Description_RemWidth    = 180 - OptionWidth - 2;
	...
	public class Command
	{
		...
		{
			var rest= Options?.Parse (arguments, true) ?? arguments;
			Run?.Invoke (rest);
			return 0;
		}
	}
	...
	public class CommandSet : KeyedCollection<string, Command>
	{
		...
		public int Run (IEnumerable<string> arguments, bool includeEnvironment)
		{
			if (arguments == null)
			...
			var extra = options.Parse (arguments, includeEnvironment);
			if (extra.Count == 0) {
				if (showHelp) {
